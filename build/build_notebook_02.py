"""Builds notebooks/02_rnns_and_their_limits.ipynb."""
from pathlib import Path
import nbformat as nbf

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "notebooks" / "02_rnns_and_their_limits.ipynb"

nb = nbf.v4.new_notebook()
cells = []

cells.append(nbf.v4.new_markdown_cell(r"""# 02 — RNNs and why they hit a wall
...
cells.append(nbf.v4.new_code_cell(r"""# --- Setup recap -----------------------------------------------------------
...
cells.append(nbf.v4.new_markdown_cell(r"""## A tiny RNN, written out
...
cells.append(nbf.v4.new_code_cell(r"""HIDDEN = 128
EMB = 32
...
cells.append(nbf.v4.new_markdown_cell(r"""## Train it
...
cells.append(nbf.v4.new_code_cell(r"""BLOCK = 64        # how far into the past the RNN gets to see per training window
BATCH = 64
STEPS = 1500
LR = 3e-3
...
cells.append(nbf.v4.new_markdown_cell(r"""Solid improvement over the bigram baseline. The RNN is using more than one character of context — just not *well*, as we'll see.
...
cells.append(nbf.v4.new_markdown_cell(r"""Real words now — "the", "and", "will", "of", short phrases with occasional local structure. Still nowhere near coherent. We could train longer and go a bit further, but we're about to hit two architectural walls that make RNNs genuinely hard to scale.
...
cells.append(nbf.v4.new_code_cell(r"""lengths = [32, 64, 128, 256, 512, 1024]
times_rnn = []
with torch.no_grad():
    for T in lengths:
...
cells.append(nbf.v4.new_markdown_cell(r"""The RNN line slopes up steeply with sequence length. The parallel-matmul line — which, notably, *is* roughly the shape of what a transformer block does — slopes up much less. This is why, when you want to train on trillions of tokens, you do not use an RNN. The training-time tax is enormous.
...
cells.append(nbf.v4.new_code_cell(r"""def grad_through_time(T=100, target_idx=0):
...
cells.append(nbf.v4.new_markdown_cell(r"""Look at the left end of that plot. The gradient reaching the earliest timesteps is many orders of magnitude smaller than at the final timestep. That's the vanishing gradient problem in a single picture.
